...
# I did this randomly and randomly adding a 1 and a 0 in the matrix but this is not simmetric and so we
# have different weights for the same line, because these networks are not directed
# def generate_matrix_with_0_and_1():
#     matrix = np.random.rand(4, 4)  # generate a 4x4 matrix with elements between 0 and 1
#     # randomly choose indices to set one element to 0 and another to 1
#     indices = np.random.choice(16, 2, replace=False)
#     matrix.flat[indices[0]] = 0
#     matrix.flat[indices[1]] = 1
#     return matrix
